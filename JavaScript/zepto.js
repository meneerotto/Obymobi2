...
//COOKIEMONSTERSCRIPT van http://www.elated.com/articles/javascript-and-cookies/
/*
...
//PRODUCTENGEDEELTE
	...
	$("button.plus").tap(function(){
	//$("button.plus").live('touchend click', function() {
		var aantal = parseFloat($(this).siblings('span').text());
		//toevoegen van 1
		aantal = aantal+1;
		//hoeveelheid plaatsen in span element
		$(this).siblings('span').html(aantal);
		//getal zichtbaar maken bij positief getal
		if (aantal == 1) 
		{
			$(this).parent().toggleClass('negatief');
		};
		...
		$('a.totaal').html('totaal '+ totaal +' items');
				
		//Cookie bijwerken voor cross-page kloppend totaal aantal
		//set_cookie("obyTotaal", totaal);
	});
	$("button.minus").tap(function() {
	//$("button.minus").live('touchend click', function() {			
		//huidige hoeveelheid ophalen
		var aantal = parseFloat($(this).siblings('span').text());
		//afhalen van 1
		if (aantal === 0) {
			...
			aantal = aantal-1;	
			//getal mag niet lager worden dan 0
			if (aantal <= 0) 
			{	//als 0 verstoppen en 0 plaatsen
				$(this).parent().toggleClass('negatief');
				aantal = 0;
				$(this).siblings('span').html(aantal);
			
			
				if ($(this).parents('ul').hasClass('productvariant') == true) {
					$(this).parent().remove();	
				}
			} else 
			{	//als boven 0, aantal plaatsen
				$(this).siblings('span').html(aantal);
				//$(this).siblings('span').anim(
				//-webkit-transform:scale(1.1);
			};
			// TOTAAL GETAL BOVENAAN PAGINA AANPASSEN
			totaal = totaal-1;		
			if (totaal <= 0) 
			{	//als 0 verstoppen en 0 plaatsen
			...
			//TOTAAL GETAL BOVEN UPDATEN MET NIEUW TOTAAL
			$('a.totaal').html('totaal '+totaal+' items');
...
//NAVIGATIE CATEGORIE NAAR PRODUCT
	$(document).ready(function()
	{
	
		//$("#categorielijst a[href='#']").attr('href', 'producten.html');
		//producten in de lijst zonder detailpagina door laten verwijzen naar algemene pagina
		$("#productlijst a[href='#']").attr('href', 'detail.html');
		
		$("#productvariant a").attr('href', '#');
		$("#detailpagina #productlijst a[href='#']").attr('href', 'detail.html');
		$("#productlijst .productmetvariant").bind('touchend', function() {
			$(this).removeClass('negatief');		
		});
		
		$("nav a").live('touchstart', function() {
			$(this).addClass('pressed');

		});
		$("nav a").live('touchmove', function() {
			$(this).removeClass('pressed');
		});
		$("nav a").live('touchend', function() {
			$(this).removeClass('pressed');
			//alert("dat was het einde");
		});

        //Place order button        $("div .backgroundDiv a").live('touchstart', function () {            $(this).addClass('pressed');        });
        $("div .backgroundDiv a").live('touchmove', function () {
            $(this).removeClass('pressed');
        });
        $("div .backgroundDiv a").live('touchend', function () {
            $(this).removeClass('pressed');
            //alert("dat was het einde");
        });
		
		//CSS voor ingedrukte list items
		$("li").live('touchstart', function() {
			$(this).addClass('pressed');

		});
		$("li").live('touchmove', function() {
			$(this).removeClass('pressed');
		});
		$("li").live('touchend', function() {
			$(this).removeClass('pressed');
			//alert("dat was het einde");
		});
		
		//CSS voor ingedrukte buttons
		$("button").live('touchstart', function() {
			$(this).addClass('pressed');

		});
		$("button").live('touchmove', function() {
			$(this).removeClass('pressed');
		});
		...
		$("button.minus").live('touchstart', function() {
			$(this).parent().addClass('minuspressed');

		});
		$("button.minus").live('touchmove', function() {
			$(this).parent().removeClass('minuspressed');
		});
		$("button.minus").live('touchend', function() {
			$(this).parent().removeClass('minuspressed');
		});
		
		$("button.plus").live('touchstart', function() {
			$(this).parent().addClass('pluspressed');

		});
		$("button.plus").live('touchmove', function() {
			$(this).parent().removeClass('pluspressed');
		});
		$("button.plus").live('touchend', function() {
			$(this).parent().removeClass('pluspressed');
		});
		
		
		$("#mainmenu a[href='#']").live('touchend', function() {
			alert("Sorry, deze pagina is nog niet uitgewerkt.");
		});
		
		//$("li.product a").before('<img height="45px" width="62px" src="http://flickholdr.com/62/44/soda,drinks/5" alt="productafbeelding"/> ');
		/*
		var totaal = parseFloat(get_cookie("obyTotaal"));
		alert(totaal);
...
$(".variant input").bind('click', function() {
	$(this).parent().toggleClass('checked');
});


$("input#variantsubmit").bind('click', function() {
	$("#productlijst").removeClass('hide');
	$("#productvariant").append('<li class="product"><span>1</span><img height="45px" width="62px" src="productimages/9845-productbutton-136w-93h-cappuccino.jpg" alt=""/><a href="#"><span>met Cacaopoeder <em>2,40 </em></span></a><button type="button" class="plus">+</button><button type="button" class="minus">-</button></li>');
	//optellen bij totaal en laten zien bij 1 of meer
		totaal = totaal+1;
		if (totaal == 1) 
		{
